...
class OrbitClaim(Document):
	...
	def create_journal_entry(self, invoices):
		...
		# Get company from the reference invoice since it's not in the Orbit Claim
		...
		je.user_remark = f"for Orbit Claim {self.name} Being {self.being}"
		...
		frappe.msgprint(f"Journal Entry {je.name} created and Orbit Claim marked as Reconciled")
